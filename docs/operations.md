...
`ledger-move-opening-lab` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies opening behavior through deny and allow fixtures, with explainable decision traces and synthetic fixtures only.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 5.
